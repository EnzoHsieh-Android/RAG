...
def run_restart_test():
    ...
    for i, test_case in enumerate(TEST_QUERIES, 1):
        ...
        if natural_success and fast_success:
            ...
            print("   📚 Fast API Top 3:")
            for j, book in enumerate(fast_books[:3], 1):
                relevance = calculate_tag_relevance(book.get("tags", []), expected_tags)
                print(f"      {j}. {book.get('title', 'Unknown')} - {book.get('author', 'Unknown')} (相關性: {relevance:.2f})")
                print(f"         標籤: {book.get('tags', [])}")
    ...
    results_summary = {
        ...
        "test_type": "restart_accuracy_test",
        "total_queries": len(TEST_QUERIES),
        "natural_api": {
            "success_rate": len(successful_natural) / len(TEST_QUERIES) if successful_natural else 0,
        ...
        "fast_api": {
            "success_rate": len(successful_fast) / len(TEST_QUERIES) if successful_fast else 0,
    ...
    with open("restart_accuracy_test.json", "w", encoding="utf-8") as f:
        json.dump(results_summary, f, ensure_ascii=False, indent=2)
    
    print(f"\n📄 重啟測試結果已保存到: restart_accuracy_test.json")
    print("🔄 重啟後精準度測試完成!")
